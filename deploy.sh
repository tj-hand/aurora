#!/bin/bash
# deploy.sh - Deploy Aurora module to Infinity and Matrix
#
# This script deploys Aurora to both frontend (Infinity) and backend (Matrix):
# - aurora-fe/* → Infinity/src/aurora/
# - aurora-be/* → Matrix/src/modules/aurora/
# - aurora-be/alembic/versions/* → Matrix/alembic/versions/
#
# Usage:
...
# ============================================================================
if [ -d "$SCRIPT_DIR/aurora-fe" ]; then
    echo "Deploying Aurora FE to Infinity..."

    FE_SLOT="$FE_TARGET/src/aurora"
    mkdir -p "$FE_SLOT"

    # Copy all frontend files
    cp -r "$SCRIPT_DIR/aurora-fe/"* "$FE_SLOT/"

    echo "  ✓ Frontend deployed to $FE_SLOT"
else
    echo "  ⚠ No aurora-fe directory found, skipping frontend"
fi

# ============================================================================
# BACKEND DEPLOYMENT (Matrix)
# ============================================================================
if [ -d "$SCRIPT_DIR/aurora-be" ]; then
    echo "Deploying Aurora BE to Matrix..."

    BE_SLOT="$BE_TARGET/src/modules/aurora"
    mkdir -p "$BE_SLOT"

    cd "$SCRIPT_DIR/aurora-be"
...
echo ""
echo "Done! Aurora module deployed."
echo ""
echo "Layer architecture:"
echo "  Aurora (Layer 4) → Mentor (Layer 3) → Guardian (Layer 2) → Matrix (Layer 0)"
echo ""
echo "Dependencies:"
...
echo "  - Email: uses Matrix email provider"
echo "  - Audit: logs to Reel"
...
echo ""
echo "Aurora API endpoints:"
echo "  GET  /api/v1/aurora/invitations           - List invitations (paginated, filtered)"
echo "  GET  /api/v1/aurora/invitations/stats     - Invitation statistics"
echo "  GET  /api/v1/aurora/invitations/{id}      - Get invitation details"
echo "  POST /api/v1/aurora/invitations           - Create invitation"
echo "  POST /api/v1/aurora/invitations/{id}/resend - Resend invitation email"
echo "  POST /api/v1/aurora/invitations/{id}/revoke - Revoke invitation"
echo "  POST /api/v1/aurora/invitations/accept    - Accept invitation (public, token-based)"
